...
fetch('https://restcountries.com/v3.1/all?fields=name,population,latlng')
    ...
    .then(data => {
        // Sort countries by population (descending) and take top 15
        countriesData = data
...
function initScene() {
    ...
    const maxPopulation = Math.max(...countriesData.map(country => country.population));
    const minPopulation = Math.min(...countriesData.map(country => country.population));
    ...
    countriesData.forEach(country => {
        ...
        const scaleFactor = (Math.log(country.population) - Math.log(minPopulation)) / (Math.log(maxPopulation) - Math.log(minPopulation));
